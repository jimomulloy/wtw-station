...
public class YahooWeatherParser implements ErrorHandler {
	...
	System.out.println("Yahoo parseForecastWeather nodeCount:"+nodeCount); 
	// iterate over search Result nodes
	for (int i = 0; i < nodeCount; i++) {
	    Element forecast = (Element) nodes.item(i);
	    // Get each xpath expression as a string
	    String date = (String) xPath.evaluate("@date", forecast);
	    String minTemp = (String) xPath.evaluate("@low", forecast);
	    String maxTemp = (String) xPath.evaluate("@high", forecast);
	    String code = (String) xPath.evaluate("@code", forecast);
	    String text = (String) xPath.evaluate("@text", forecast);
